 ...
 * @OnOpen
 *      当websocket建立连接成功后会触发这个注解修饰的方法，注意它有一个Session参数
 * @OnClose
 *      当websocket建立的连接断开后会触发这个注解修饰的方法，注意它有一个Session参数
 * @OnMessage
 *      当客户端发送消息到服务端时，会触发这个注解修改的方法，它有一个String入参表明客户端传入的值
 * @OnError
 *      当websocket建立连接时出现异常会触发这个注解修饰的方法，注意它有一个Session参数
 */
@Slf4j
@Tag(name = "直播间即时通信接口")
...
public class WebSocketApiController {
    ...
    public void onOpen(Session session, @PathParam("liveRoomId") Long liveRoomId, @PathParam("token")String token){
        ...
        // 添加到容器工具类
        WebSocketLocalContainer.addUserToLiveRoom(liveRoomId, userInfoVo.getId());
        WebSocketLocalContainer.addFromUser(userInfoVo.getId(), fromUser);
        WebSocketLocalContainer.addSession(userInfoVo.getId(), session);
...
//        WebSocketLocalContainer.sendMsg(socketMsg);
    ...
    public void onClose(Session session, @PathParam("liveRoomId")Long liveRoomId, @PathParam("token")String token){
        ...
        SocketMsg socketMsg = new SocketMsg();
        socketMsg.setMsgType(SocketMsg.MsgTypeEnum.JOIN_CHAT.getCode());
        socketMsg.setMsgContent(userInfoVo.getNickname() + "离开了直播间！");
        socketMsg.setTime(new DateTime().toString("HH:mm:ss"));
        // 构建用户基本信息
        socketMsg.setFromUser(fromUser);
        socketMsg.setLiveRoomId(liveRoomId);
